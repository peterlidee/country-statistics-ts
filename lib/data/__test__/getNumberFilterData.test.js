...
describe('Running all functions with mock data', () => {
  ...
  const mock2 = [
    filterDataMock.population.countryMin,
    filterDataMock.population.countryMax,
  ]
  const mock3 = [
    filterDataMock.density.countryMin,
    filterDataMock.density.countryMax,
  ]
  ...
  const mock1StepsNeeded = mock1Closest.map((option) =>
    stepsNeeded(option, mock1[0], mock1[1]),
  )
  const mock2StepsNeeded = mock2Closest.map((option) =>
    stepsNeeded(option, mock2[0], mock2[1]),
  )
  const mock3StepsNeeded = mock3Closest.map((option) =>
    stepsNeeded(option, mock3[0], mock3[1]),
  )
  ...
  describe('Run calculateNumberSliderSetting with mock data', () => {
    const mock1Calculations = calculateNumberSliderSetting(
      mock1[0],
      mock1[1],
      15,
    )
    const mock2Calculations = calculateNumberSliderSetting(
      mock2[0],
      mock2[1],
      15,
    )
    const mock3Calculations = calculateNumberSliderSetting(
      mock3[0],
      mock3[1],
      15,
    )
