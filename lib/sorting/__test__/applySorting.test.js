...
describe('function applySorting', () => {
  ...
  test('It returns the defaults when !routerReady', () => {
    const result = applySorting(false, {}, [1, 10, 100])
    expect(result).toEqual({
      sortBy: 'country',
      sortAsc: true,
      countries: [1, 10, 100],
    })
    expect(getParameterFromQuery).not.toHaveBeenCalled()
    expect(validateSortQuery).not.toHaveBeenCalled()
    expect(sortCountries).not.toHaveBeenCalled()
  })

  test('It returns the mock return values when routerReady', () => {
    const result = applySorting(true, { query: 'foobar' }, [1, 10, 100])
    expect(result).toEqual({
      sortBy: 'foobar',
      sortAsc: true,
      countries: [3, 2, 1],
